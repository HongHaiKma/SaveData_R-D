...
public static class EzEventManager
{
    ...
    public  static void TriggerEvent<T>(T eventType)
    {
        MMEventManager.TriggerEvent<T>(eventType);
    }
}
#endif
...
public static class Vector2Ultis
{
    public static  float ParabolLength ( Vector3 start, Vector3 end, float height)
    {
        var length = 0.0f;
        var oldPoint = start;
        for (int i = 1; i < 100; ++i)
        {
            var newPoint = SampleParabola(start, end, height, 0.01f * i);
            length += Vector2.Distance(newPoint, oldPoint);
            oldPoint = newPoint;
        }
...
public static class ArrayExtension
{
    ...
    public static T[] FindAll<T>(this T[] array, Predicate<T> Predicate)  where T : class
    {
        return  array == null ? null : Array.FindAll(array, x => x != null && Predicate(x));
    }
    public static T Find<T>(this T[] array, Predicate<T> Predicate)  where T : class
    {
        return  array == null ? null : Array.Find(array, x => x != null && Predicate(x));
    }
    public static  List<T> FindQuantity<T>(this List<T> array,int quantity, Predicate<T> Predicate)  where T : class
    {
        ...
        foreach (var VARIABLE in array)
        {
            ...
            {
                if(results.Count >= quantity && quantity > 0)
                {
                    break;    
                }
...
{
    public enum FacingDirectionRotate
    {
        UP = 270,
        DOWN = 90,
        LEFT = 180,
...
public static class TimeExtension
{
    ...
    public static IEnumerator GetNetTime(System.Action<string, string> pResult)
    {
        ...
        string url = "https://trmario.tqcstudio.com/TimeAPI/GetCurrentTime";
...
{
    public static string ReplaceStringBetween(this string main, string a, string b,string replacement)
    {
        string replacedString = main;
       var indexFirst = main.IndexOf(a, StringComparison.Ordinal);
       var indexSecond = main.IndexOf(b, StringComparison.Ordinal);
       var midText = replacedString.Substring(indexFirst + a.Length, indexSecond - indexFirst - a.Length);
       replacedString = replacedString.Replace(midText, replacement);
       return replacedString;
    }
}
public static class ImageExtension
{
    public static void SetSprite(this Image image, Sprite sprite, Vector2 claim)
    {
    
        if (claim == Vector2.zero)
        {
            image.sprite = sprite;
            return;
        }
        var cacheScale = image.transform.localScale;
        image.sprite = sprite;
        ...
        image.transform.localScale = cacheScale;
        if (!sprite)
        {
