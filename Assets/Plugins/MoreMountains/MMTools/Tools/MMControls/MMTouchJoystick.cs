...
namespace MoreMountains.Tools 
{
	...
	{
		public static MMTouchJoystick _instance;
		...
		public UnityEvent JoystickRelease;
		
		public bool SendEventWhenRelease = true;
		...
		//	_canvasGroup.alpha = 0.3f;
			_knobTransform.transform.position = defaultPos;
			joystick.transform.position = defaultPos;
		}

		public void enableJooystick()
		{
		...
		{
            if (JoystickValue != null && (SendEventWhenRelease || _joystickValue != Vector2.zero))
			{
		...
		{
			inputDirection = _knobTransform.transform.localPosition - joystick.transform.transform.localPosition;
			// we reset the stick's position
			_newJoystickPosition = _neutralPosition;
			_newJoystickPosition.z = _initialZPosition;
			_knobTransform.position = defaultPos;
			joystick.transform.position = defaultPos;
			_joystickValue.x = 0f;
			_joystickValue.y = 0f;
		...
		public void OnPointerDown(PointerEventData eventData)
		{
			...
			_knobTransform.transform.position = TargetCamera.ScreenToWorldPoint(eventData.position).MMSetZ(0);
			joystick.transform.position = TargetCamera.ScreenToWorldPoint(eventData.position).MMSetZ(0);
			SetNeutralPosition();
			var dir = joystick.transform.parent.TransformVector(inputDirection);
			var floatValue = Mathf.Clamp( dir.magnitude / MaxRange,0,MaxRange);
			joystick.transform.localPosition   -= (Vector3)inputDirection *( floatValue);
			_neutralPosition -= (Vector2)dir;
			cachePos = _knobTransform.position;
		}
